"""Independently parse, check and render the exported EVT E2 CAM files."""
...
def inspect(folder, expected_holes):
    ...
    lands = [obj for obj in layers["top", "copper"].objects
             if type(obj).__name__ == "Flash" and ".P" in obj.attrs
             and obj.attrs[".P"][0] in refs]
    assert len(refs) == 122 and len(lands) == 410 and len(paste) == 413
    ...
    for index in range(1, 17):
        assert pd_pins[f"PD{index}"] == {
            "1": "AGND", "2": f"PD_IN{index}"}
    ...
    assert "TF.FileFunction,Plated" in next(source.glob("*-PTH.drl")) \
        .read_text()
    assert "TF.FileFunction,NonPlated" in next(source.glob("*-NPTH.drl")) \
        .read_text()
    expected = {(True, 0.3): 264, (False, 0.991): 3}
    ...
    for name, (side, use, color) in VIEWS.items():
        colors = {f"{side} {use}": color,
                  "mechanical outline": "#e04925",
                  "drill pth": "#ffffff", "drill npth": "#111111"}
        svg = stack.to_svg(margin=1, side_re=f"{side}|mechanical",
                           colors=colors,
                           drills=use != "paste",
    ...
    result = {
        ...
        "paste_note": "410 lands; U1 exposed pad uses 4 paste windows, "
        "so the stencil has 413 openings.",
        ...
        "drills": [{"plated": key[0], "diameter_mm": key[1], "count": value}
...
for ring in bare.interiors:
    hole = Polygon(ring)
    match = min((Polygon(r) for r in panel.interiors),
...
print(json.dumps({"unit_holes": 2, "carrier_holes": 10,
                  "eye_hole_hausdorff_mm": eyes,
                  "unit_drills": unit["drill_total"],
                  "carrier_drills": carrier["drill_total"]}, indent=2))
