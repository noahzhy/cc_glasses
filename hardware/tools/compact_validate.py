...
report = {
    ...
    'board_dimensions_mm': [141.73, 71.5, 1.6],
    'copper_layers': 2,
    'footprints': len(list(b.GetFootprints())),
    'tracks': len(tracks), 'vias': len(vias), 'zones': len(list(b.Zones())),
    ...
    'sensitive_input_total_trace_lengths_mm': {
        n: round(v, 2) for n, v in sorted(lengths.items())
        if n.startswith('PD_IN')},
    'status': 'EVT prototype; bench, optical and mechanical validation pending',
}
assert not any(report[n] for n in ['erc_violations', 'drc_violations',
                                 'unconnected_items', 'schematic_parity_issues'])
