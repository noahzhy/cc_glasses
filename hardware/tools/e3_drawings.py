...
def outline(ax):
    ax.add_patch(
        Polygon(outer, facecolor="#e0ece2", edgecolor="#294633", linewidth=0.8)
    )
    for hole in holes:
        ax.add_patch(
            Polygon(
                hole, facecolor="white", edgecolor="#294633", linewidth=0.8
            )
        )
    ax.set_aspect("equal")
    ax.invert_yaxis()
...
with PdfPages(ROOT / "optical_assembly.pdf") as pdf:
    for eye, start, limits, center in [
        ...
        ax.set_ylim(112, 56)
        refs = {
            f"{p}{n}" for p in ["D", "PD"] for n in range(start, start + 8)
        }
        for ref in sorted(refs):
            ...
            for pad in [
                p for p in items if p["kind"] == "pad" and p["ref"] == ref
            ]:
        ...
        fig.suptitle(
            f"EVT E3 | {eye} OPTICAL ASSEMBLY | TOP VIEW",
            fontsize=14,
            x=0.08,
            ha="left",
        )
...
fig.suptitle(
    "EVT E3 | FINISHED BOARD DIMENSIONS | mm | TOP VIEW",
    fontsize=14,
    x=0.065,
    ha="left",
)
fig.text(
    ...
    "Straight upper edge: previous 1.5 mm power pocket removed. Dimensions refer to "
