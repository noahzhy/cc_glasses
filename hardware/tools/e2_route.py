...
step = float(os.environ.get("E2_ROUTE_GRID", "0.05"))
...
failed = set()
for net in [name for name in sys.argv[1:] for _ in range(8)]:
    if net in failed:
        continue
    own = [i for i, item in enumerate(items) if item["net"] == net]
    extent = unary_union([shapes[i] for i in own]).buffer(5)
    ...
    via_blocked |= raster(unary_union(lands).buffer(0.18))
    for item in items:
