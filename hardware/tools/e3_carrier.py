"""Build a removable 1-up carrier around the released EVT E3 board."""
...
def geometry():
    ...
    board = translate(
        Polygon(source["outer"], source["holes"]),
        xoff=ORIGIN[0],
        yoff=ORIGIN[1],
    )
    ...
    for x in (60.0, 95.0, 125.0, 160.0):
        edge = board.exterior.intersection(
            LineString([(x, top - 1), (x, top + 8)])
        )
        points = list(getattr(edge, "geoms", [edge]))
        y = min(point.y for point in points)
        tabs.append(box(x - 1.5, top - 3, x + 1.5, y + 1))
    for x in (74.0, 146.0):
        edge = board.exterior.intersection(
    ...
    keepouts = [
        board.exterior.intersection(tab).buffer(0.3, cap_style=3, join_style=2)
        for tab in tabs
    ]
    ...
    data = {
        ...
        "cut_zone_keepouts": [list(area.exterior.coords) for area in keepouts],
        "npth_centers_mm": [
            (x0 + 4, y0 + 4),
            (x1 - 4, y0 + 4),
            (x0 + 4, y1 - 4),
...
def drawing(data, board, tabs):
    ...
    def path(rings):
        return " ".join(
            "M " + " L ".join(f"{x:.4f},{y:.4f}" for x, y in ring) + " Z"
            for ring in rings
        )
    ...
    parts = [
        ...
        f'<path d="{path(board_rings)}" fill="#bfdcc7" stroke="#236d3b"/>',
    ]
    for tab in tabs:
        parts.append(
            f'<path d="{path([tab.exterior.coords])}" '
    ...
    for x, y in data["npth_centers_mm"]:
        parts.append(
            f'<circle cx="{x}" cy="{y}" r="1" fill="white" stroke="#333"/>'
        )
    for x, y in data["fiducial_centers_mm"]:
        parts.append(
            f'<circle cx="{x}" cy="{y}" r="1" fill="white" stroke="#777"/>'
        )
        parts.append(
            f'<circle cx="{x}" cy="{y}" r="0.5" fill="#b08419" stroke="none"/>'
        )
    parts.extend(
        [
            '</g><g font-family="Arial,sans-serif" font-size="2" fill="#222">',
            f'<text x="{x0}" y="{y0 - 6}">'
            "EVT E3 - 1-up removable SMT carrier</text>",
            f'<text x="{x0}" y="{y0 - 2}">'
            ...
            f'<text x="{x0}" y="{y1 + 8}">'
            "Support board while routing tabs; retain the final outline. "
            "No V-cut.</text>",
            f'<text x="{x0}" y="{y1 + 12}">'
...
def build_board():
    ...
    node = [
        ...
        or not any(
            ...
            and len(field) > 1
            and str(field[0]) == "layer"
            and field[1] == "Edge.Cuts"
            for field in item
        )
    ]
    ...
    shutil.copyfile(
        ROOT / "ir_glasses.kicad_pro", OUT / "ir_glasses_carrier.kicad_pro"
    )
    ...
    for name in ("fp-lib-table", "sym-lib-table"):
        ...
        (OUT / name).write_text(
            text.replace("${KIPRJMOD}/", "${KIPRJMOD}/../"), encoding="utf-8"
        )
...
def main():
    ...
    if not args.geometry_only:
        subprocess.run(
            [str(KICAD / "bin/python.exe"), __file__, "--board-stage"],
            check=True,
        )
