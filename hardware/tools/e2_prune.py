...
unused = {item["uuid"] for v in report["violations"]
          if v["type"] == "via_dangling" for item in v["items"]}
...
for iteration in range(1):
    ...
    for track in list(board.GetTracks()):
        ...
        for other in connectivity.GetConnectedTracks(track):
            width = (pcb.Cast_to_PCB_VIA(other).GetWidth(pcb.F_Cu)
                     if other.GetClass() == "PCB_VIA" else other.GetWidth())
            clearance = (track.GetWidth() + width) / 2 + 1000
            for pos in [other.GetStart(), other.GetEnd()]:
                along = ((pos.x - a.x) * dx + (pos.y - a.y) * dy) / length2
                along = max(0.0, min(1.0, along))
                gap = math.hypot(pos.x - a.x - along * dx,
                                 pos.y - a.y - along * dy)
                if gap <= clearance:
...
for node in tree[:]:
    ...
    if actions[uid] is None:
        ...
        for key, value in zip(["start", "end"], actions[uid]):
            item = next(x for x in node if isinstance(x, list)
                        and x and str(x[0]) == key)
            item[1:3] = value
if actions:
