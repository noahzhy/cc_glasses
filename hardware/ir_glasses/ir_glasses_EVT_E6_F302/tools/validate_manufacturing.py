...
check('155 unique fitted BOM references',len(bom)==len(refs)==155)
counts={}
for sub in ['', 'carrier']:
 folder=ROOT/sub;name='ir_glasses_EVT_E6_F302'+('_carrier' if sub else '');b=sx.load(open(folder/(name+'.kicad_pcb')))
 ...
 check(sub+' CPL exactly matches fitted BOM',len(cpl)==155 and {r['Designator'] for r in cpl}==refs)
 ...
 counts[sub or 'unit']={'copper_layers':list(copper),'drills':actual,'outline_segments':len(edge.objects),'fitted_parts':155}
...
(ROOT/'manufacturing_validation.json').write_text(json.dumps({'status':'passed','checks':checks,'counts':counts,'board_hashes':{str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in [ROOT/'ir_glasses_EVT_E6_F302.kicad_pcb',ROOT/'carrier/ir_glasses_EVT_E6_F302_carrier.kicad_pcb']},'cam_hashes':{str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for folder in [ROOT/'manufacturing',ROOT/'carrier/manufacturing'] for p in folder.iterdir() if p.is_file()},'not_tested':['factory DFM','SMT platform rotation corrections','bench optical/electrical performance']},indent=2));print(len(checks),'CAM/BOM/CPL/library checks passed')
