...
完整解压后打开 ir_glasses.kicad_pro。此版本直接重命名并重新关联项目库，不存在另一个无库关联的导入工程。

本地库位于 libraries/；封装名例如 C:0402、R:0402、IC:TSSOP-14。
IR 和 PD 独立成库，均可使用短名 1206，保留各自不同的焊盘和极性。
符号库名为 S。全部库路径为相对路径，可连同工程迁移。

嘉立创用配套 JLC_upload.zip 内的 BOM、坐标和 Gerber；工程及全部库也包含在本交付包中。
库链接、ERC、DRC、几何一致性证据见 library_link_validation.json、library_migration.json 和 pre_smt_final_check.md。
