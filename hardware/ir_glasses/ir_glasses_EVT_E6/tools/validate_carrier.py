...
for path in ['drc.json','carrier/drc.json']:
 d=json.load(open(ROOT/path))
 for key in ['violations','unconnected_items','schematic_parity']:check(path+' '+key+' empty',not d.get(key))
result={'status':'passed','checks':checks,'check_count':len(checks),'unit_sha256':hashlib.sha256((ROOT/'ir_glasses_EVT_E6.kicad_pcb').read_bytes()).hexdigest(),'carrier_sha256':hashlib.sha256((ROOT/'carrier/ir_glasses_EVT_E6_carrier.kicad_pcb').read_bytes()).hexdigest(),'bench_testing':'pending'}
(ROOT/'carrier_validation.json').write_text(json.dumps(result,indent=2));print(len(checks),'carrier preservation checks passed')
